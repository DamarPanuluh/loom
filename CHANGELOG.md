...
## [0.35.1] - 2026-08-21
- route needed findings to the lane that can write the named repair
...
- the proof harness tracks re-entrancy per call stack, closing a thread-safety bug any parallel suite could hit
  - The marker was a process-global environment variable that nothing ever cleared. `cargo test` runs tests as threads in one process, so a sibling thread locking an unrelated root overwrote it; the first thread then failed its own re-entrancy check, tried to lock a path it already held, and was refused by itself — reported as contention with its own pid. It is now a `thread_local!` set with a real `Drop`. A different thread wanting the same lock still meets the file lock and is still refused.
- a blocked Journey run names the typed check that failed instead of counting it
  - `RuntimeReport.failed_assertions` records the operation, assertion id, pointer and kind for assertion misses and for captures that could not be taken, covering the setup path too. Release diagnostics carry it, so a candidate that refuses no longer costs a sealed authority token just to learn which check moved. Empty and omitted for passing runs, so a passing report is byte-identical.
- `loom journey lint` computes what is actually ambient, and sees identities it previously missed
  - Counts and reply indices over a transcript the operation itself authored are exempt, as is `/result/content/0` — `mcp::tool_content` always emits exactly one element, and a different content index still fires. `graph-local-identity` now recognises 17-hex derived ids and ids nested inside JSON arguments. New blocking rule `stale-compiler-version-pin` compares a pinned compiler version against the real one. Advisories across the shipped fixtures fell from 326 to 80, and every survivor is classified in `docs/journey-authoring.md`.
- `sync` reports the grounding share of a ripple, and a finding resolves by exact unique name
  - `groundings_staled` is the `implements` share of `edges_staled`, derived from the recorded set rather than counted per staling site: the union count depends on how much of the validation plane is settled, so the same edit re-opens one grounding locally and over a hundred edges in a freshly imported graph. `resolve_finding` also accepts an exact, unique name — fuzzy matching and fragments stay refused — because an asserted finding's id is random, leaving a caller that mints a finding and then adjudicates it no other stable handle.
  - The release inventory no longer pins `entry_count` and `file_count` to the repository's exact file total, which would have refused the release proof the next time any file was added.

## [0.32.0] - 2026-08-14
- add operation-level `expected_exit` so structured-failure CLIs that exit non-zero can prove their JSON envelope as an observed Journey result; schema v12 graphs require no rebuild
  - Surface operations now accept an optional non-negative `expected_exit` integer (omitted or `0` keeps the exit-0 rule). The runtime treats the exact expected exit as process liveness: a production CLI that writes its single JSON envelope to stdout and then exits with the declared non-zero code (for example a `gridctl` exiting 11 on a 401) is now an observed result — its compiled captures and assertions run against the envelope and its executable boundary is recorded, so settlement no longer hits "executed operation boundary does not match the compiled proof" for failed children. Killed or signaled processes, timeouts, wrong exit codes, and non-JSON stdout still fail the run through the same bounded diagnostic; negative values are rejected at parse time.
  - `CompiledStep` and `CompiledSetupOperation` carry the field with a zero default and skip-when-zero serialization: compiled proofs written before this change deserialize unchanged, and surfaces that never set `expected_exit` compile to byte-identical proofs, so the Journey compiler version stays **6** and no recompilation is required.
  - Exit status stays out of assertions: `/ok`, `/error.kind`, and `/error.code` remain JSON assertions in `output.assertions`, and the target CLI keeps its real exit-code contract (no proof-only wrappers or fail-ok flags). Documented on `CliOperation` and in `docs/journey-authoring.md`.
  - `journey run --json` failures are now machine-readable on stdout: the command prints one `{status: "error", journey, profile, stage, detail}` envelope before exiting non-zero (stage is `compile`, `open`, or `settle`), so consumers parse the reason instead of scraping stderr.
  - Boundary-mismatch settlement refusals now name the offending step and both argv0 tokens (for example "step 'checkout' (operation 'gridctl-reject') declares argv0 'gridctl' but the run recorded 'shim'") instead of two opaque sorted pair lists.
- regenerate the committed v12 dogfood export so the release-workflow surface matches its accepted manifest
  - The committed `loom.graph.json` still carried the pre-`70adc4e` release-workflow surface body, so the release gate refused `surface-accept` with "interface surface 'loom-release-rehearsal' already exists with a conflicting definition" (three stale `/source_inventory` assertion values). The export was rebuilt from a scratch store with the current sources; the dogfood and fixpoint gates are green again.
