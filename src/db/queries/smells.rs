...
const TEACHING_TABLE: &[(&str, &str, &[&str], &[&str], &str)] = &[
    ...
    ("symbol_accountability_gap", "Behavior-significant symbols should be owned, accepted, or turned into explicit work; raw helper coverage is not the target.", &["`loom coverage --json` and read actionable_symbol_gaps", "`loom codefile show <path>` for each top gap before changing graph ownership", "decide whether the symbol needs a precise locator, a split intent, or a decision note accepting broad ownership"], &["do not chase 100% raw symbol coverage", crate::db::queries::symbol_accountability::ANTI_CREATE_INTENTS_PER_HELPER, crate::db::queries::symbol_accountability::ANTI_BULK_GROUND_SYMBOLS], "actionable symbol gaps are grounded with precise locators, accepted with a current decision note, or converted into real intent split/build work"),
