...
pub fn list_implements_for_intent(db: &dyn LoomDb, intent_id: &str) -> Result<Vec<Implements>> {
    let q = format!(
    ...
    Ok(result
        .rows()
        .iter()
        .map(|row| row_to_implements(row, &cols))
        .collect())
}
...
pub fn intents_with_implements(db: &dyn LoomDb) -> Result<std::collections::HashSet<String>> {
    let r = db.execute("MATCH (i:Intent)-[e:IMPLEMENTS]->(:CodeFile) RETURN i.id AS iid")?;
    let cols = col_map(&r);
    Ok(r.rows()
        .iter()
...
#[cfg(test)]
pub fn intent_ids_implementing_codefile(db: &dyn LoomDb, codefile_id: &str) -> Result<Vec<String>> {
    let q = format!(
