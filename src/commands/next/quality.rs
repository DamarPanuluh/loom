...
pub(super) fn run_take_quality(
    ...
    let guidance = "Per group: read the intent's grounded code ONCE (`loom intent show <id>` lists files + locators), hold each rule against it, edit its template line — keep `passing` with real evidence, `failing` with the violation, `independent` when the rule has no surface here (as valuable as passing — never fake it) — then apply the whole group in ONE call: paste the edited lines into a heredoc, `loom batch - <<'EOF' … EOF` (no scratch file, nothing to clean up; a file path works for very large batches). Template lines omit `criterion` when one is recorded: `loom batch` reuses it; write a criterion only to revise it. Prose values are plain text, not Markdown: do not use backticks; name commands and symbols in words. A verdict at component altitude covers descendants ONLY with --covers-descendants: if every rule reads the same for the whole subtree, verdict the parent with --covers-descendants instead; without it the verdict covers only the parent.";
