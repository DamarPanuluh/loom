...
use crate::db::queries::{get_layer_order, list_active_intents, set_layer_order};
...
fn run_inner(cmd: LayerCmd, printer: &Printer, deprecated_alias: bool) -> Result<()> {
    ...
    let alias_note = deprecated_alias.then_some(
        ...
        LayerCmd::Order { layers, author } => {
            ...
            for layer in &layers {
                ...
                if !seen.insert(layer.as_str()) {
                    anyhow::bail!(
                        "Layer '{layer}' appears twice — each layer holds exactly one rank."
                    );
                }
            }
