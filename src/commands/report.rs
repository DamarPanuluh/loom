...
pub fn run(printer: &Printer) -> Result<()> {
    ...
    sorted_statuses.sort_by(|(a, _), (b, _)| a.cmp(b));
    for (s, count) in &sorted_statuses {
        println!("  {s:<30}  {count}");
    }
    println!();

    println!("── Intents with No Validations ({}) ─────────────────────────────────", intents_no_val.len());
    if intents_no_val.is_empty() {
        ...
        println!();
        println!("  → Add validations with `loom validation add` and link via `loom edge validates`.");
    }
    println!();

    println!("── Failing GOVERNS Edges ({}) ──────────────────────────────────────", failing_governs.len());
    ...
    println!(
        ...
        vc.roots,
        vc.leaves,
        if vc.multi_parent.is_empty() && !vc.cycle { "well-formed" } else { "MALFORMED — see `loom doctor`" },
    );
    ...
    if !vc.non_system_roots.is_empty() {
        println!("  ⚠ root intents not at `system` level (advisory — does not block completeness):");
    ...
    println!("── Completeness Gaps ({}) ───────────────────────────────────────────", gaps.len());
    if gaps.is_empty() {
