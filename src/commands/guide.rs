    ...
    if printer.json {
        printer.print_json(&serde_json::json!({
            ...
            "completeness": {
                ...
                "horizontal": "REQUIRED for full-green (`phase=complete`/`fully_proven`), though NOT for the vertical spine: every intent pair has an inspected RELATES_TO edge (passing/failing/independent). Surfaced as `horizontally_explored`. `loom edge unexplored` lists what's owed; mark `independent` at the highest honest altitude (a component verdict covers its descendants).",
            },
