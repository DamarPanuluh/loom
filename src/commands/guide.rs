...
const GOLDEN_RULES: &[&str] = &[
    ...
    "THE CONSUMER PLANE (runtime proof of composition): everything else grounds claims by READING code — a saga proves intents compose by EXECUTING them the way a real consumer will. Write a YAML spec (ordered endpoint chain; every step names the intent it proves; captures thread one response into the next request), `loom saga add <spec.yaml>` to declare it (Validation type=saga + VALIDATES edges + the RELATES_TO path), `loom saga diagnose <name>` to TRIAGE without stamping graph verdicts, and `loom saga run <name>` to STAMP proof: consecutive passing steps stamp their RELATES_TO edge passing with RUNTIME evidence; the boundary into a failing step goes failing with the exact broken expectation ('expected 200, got 502'); never-reached steps stay untouched. Validator lane; exits non-zero so it runs under `loom validate`/CI; sync re-queues it when step-intent code changes. ENVIRONMENT VALUES: `{{ env.X }}` in a spec means the value arrives AT INVOCATION — `BASE_URL=http://localhost:3000 loom saga diagnose <name>` for diagnosis, then `BASE_URL=http://localhost:3000 loom saga run <name>` to record evidence — never stored in the graph (it points at a LIVE target; start the system under test first — discover how THIS repo brings it up: docker-compose, a Makefile/justfile target, scripts/, package.json scripts, the README — loom ships no mock, it drives the real composition). `loom saga add`/`list` name what's required (`run with: BASE_URL=<value> …`); a missing value refuses to run with the exact invocation to use (nothing stamped — environment-not-ready is never recorded as a failed proof: `loom validate` marks it `blocked` instead). Use it for any endpoint-reachable surface; deliberately not a general HTTP test tool.",
...
fn brownfield() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        ("intake boundary", "Free-form human/LLM language enters through Inbox first: `loom door \"<utterance>\"` captures an InboxItem and serves routing context; `loom inbox triage --take 20` normalizes cards into exact proposed commands. Existing structured graph commands stay valid once the fields are already known."),
