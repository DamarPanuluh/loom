...
fn seed() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        ("the align loop", "On a populated graph, `loom next --mode align` ranks drift suspicion: code churn since last user confirm × centrality × staleness. TRANSLATE, never parrot: the intent's description is graph-speak written for grounding code — reading it aloud confuses the exact non-coder this loop serves. Re-express what it makes the product DO from the user's vantage, one or two plain sentences (jargon test: would a non-coder nod? file paths, route lists, internal nouns = translate again), then ask if that's still what this part should do — their answer is about BEHAVIOR, not wording. Record exactly ONE outcome: still right → `loom intent confirm <id>`; evolved → translate their words BACK into a falsifiable description, `loom intent update <id> --description … --reason …`; superseded → `loom intent retire <id> --reason … --replaced-by <successor>`; revealed gap → `loom intent add … --lifecycle planned`. A laundry-list meaning that needs 'and' is itself a finding — propose the split. Confirm resets the suspicion clock."),
