...
fn seed() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        ("one question, one landing", "Ask ONE question at a time, always with your recommended answer. If code can answer it, switch to brownfield and explore instead of asking (`loom guide --mode brownfield`). The moment an answer crystallises, LAND it before the next question — with the rung's altitude AND its audience in one shot (`--visibility user_visible|internal`, plus `--boundary inbound|outbound` when it crosses the edge): behavior → `loom intent add … --lifecycle planned`; term → `loom vocab add`; hard tradeoff → `loom note add --kind decision`; error path → same tree with `--aspect sad|fallback`. Atomic only: if an intent description needs 'and', split it."),
