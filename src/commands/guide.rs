...
const DEEPER_RULES: &[&str] = &[
    ...
    "360°: grounded · realized · measured · proven. `measured` never closes alone: `loom rule seed iso5055|mobile|web-ui|service|data|concurrency|docker` → `loom next --mode quality`.",
    ...
    "COMPOSE TIER: `loom paths` shows composition-proof coverage — path-proven (a saga, or a proof spanning >=2 intents, covers it) / leaf-only (proven locally, no journey) / unproven. INFORMATIONAL, never gates: judge each leaf-only — is a real journey missing, or a genuine terminal leaf? The journey GATE is the `unjourneyed_surface` smell: user-visible code needs a passed consumer-journey proof — resolve it by adding a saga, marking `visibility=internal`, or recording why no journey applies.",
    "ROLE ≠ MODE: a ROLE (`LOOM_AGENT` / `loom guide --role`: builder/analyzer/fixer/validator/quality) is WHO you are — the discipline that GATES which fields you may write; a MODE (`loom next --mode`: build/discovery/fix/validate/quality/review/prove/refactor/align) is WHICH work-queue you drain. Each role maps to its lane (builder→build, analyzer→discovery, fixer→fix, validator→validate, quality→quality) — the names overlap but the axes differ: role is your hat, mode is your queue.",
...
const RIPPLE: &[&str] = &[
    "RELATES_TO edges of intents grounded in the changed file → needs_reverification (re-inspect via `loom next --mode fix`; the edge's transition note names the changed file), with one exception: an `independent` edge re-opens only when new structural coupling appears between its endpoints (a new import or new shared grounding). A behavior-preserving edit that adds no new coupling does not re-open an independence verdict. Edges carrying a judgment coupling (calls/inheritance/shares_state) DO re-open when their owning file changes",
...
const ROLE_DISCIPLINE: &[(&str, &str, &str, &[&str])] = &[
    ...
    ("analyzer",
     ...
     &[
        ...
        "BULK: `loom next --mode discovery --take 50` groups derived coupling candidates (import-linked / asserted-stale pairs) with both intents + groundings inline — read each neighborhood ONCE, apply the whole group in one `loom batch -`. `loom cluster <intent>` lists every unresolved edge on one node. The engine proposes derived candidates; your job is to confirm or declare independent with evidence.",
...
fn brownfield() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        ("discover", "`loom next` repeatedly: read the code it points to, then record `loom edge explore <a> <b> ground|issue|independent …`. Imports reveal only STATIC wiring — also read for INDIRECT wiring (event pub/sub, DI/registry, config-keyed dispatch, RPC/queue: a shared topic/route/token string or type with NO import between the files). Ground a real coupling with kind `manual` and the shared key as evidence; prove the runtime path with a saga; if the wiring looks INCOMPLETE, capture it (`loom inbox add \"<finding>\" --source code_audit --link file:<path>`) instead of deciding inline."),
...
fn seed() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        ("challenge, don't transcribe", "When the user's term collides with registered vocab, call it out and resolve with `loom vocab add` or a decision note (`loom note add --kind decision`). When a claim contradicts existing intents or code, surface the contradiction and make the user choose. Treat the graph as a decision aid, not the boss: if the user makes an explicit PM boundary call, assert the contradiction back to them, then encode their product decision in the intent instead of hiding behind old graph labels. Example: \"privacy page is an inbound operator UI boundary because it is a routed Svelte surface, not backend internals\" should become an intent confirm/update with `--visibility user_visible --boundary inbound` and wording that names the route/surface. Stress-test boundaries with scenarios: \"a payment fails mid-checkout — what does the user see?\" Each answer usually lands as `loom intent add … --aspect sad|fallback --lifecycle planned`."),
    ...
    if printer.json {
        printer.print_json(&serde_json::json!({
            ...
            "completeness": {
                ...
                "horizontal": "Feeds the Hardened rung (not the vertical spine): every explicit RELATES_TO edge is inspected and current (`horizontally_explored`), and coupling risk is gated by the smells (undeclared_coupling / overlapping_ownership / duplicated_responsibility / twin_intents). The N×N missing-pair grid was SUPERSEDED — it no longer gates: `loom edge unexplored --class suspected-coupling` is OPTIONAL discovery, ranked by centrality (`--class all` is the exhaustive optional survey).",
            },
            "done_condition": "TWO VIEWS of one journey, NEVER conflated: (1) the COMPASS PHASE (`graph_state.phase` — shape → realize → complete → harden → green) is the single most-urgent NEXT MOVE: the first unmet STRUCTURAL gate, carried as `next_action` (and as `loom next`'s directive when the gate has no queue, e.g. complete=disk, harden=smells). (2) the MATURITY LADDER (the rungs below) is your STANDING — the certification vector. The PHASE says what to DO right now; the LADDER says where you STAND; when they point differently, the phase surfaces the most-urgent structural gate while the ladder shows the fuller climb (and the ladder's FOCUS lane is what `loom next` drains once the compass gate is clear). The two share a progression but are NOT the same list — never call a phase a 'rung'. The MATURITY LADDER is a certification vector, not just a completion scalar (`loom status` / `loom complete` → `maturity.rungs`) with a FOCUS = the lowest unmet rung, where `loom next` routes. Ordered by RECORD ≠ DISCHARGE: SEEDED (every responsibility captured; entrypoint owned, inbox triaged) → REALIZED (every leaf grounded + proven by an EXECUTED discriminating test, `proven_executed == realized`, no doc-only spec-as-built) → PROVEN (every user_visible journey has a passing boundary proof — saga or human manual_check; N/A and collapses when there are no journeys) → HARDENED (measured under rules, RELATES_TO risk closed, failure-path siblings realized, ZERO open `loom smells` findings) → PRODUCTION-READY (all lower rungs cleared + wiki fresh, inbox drained, boundary owned — deploy-fitness) → EXCELLENT (Production-ready plus zero unresolved excellence debt: refactor/design/proof-locality debt is fixed, false-positive, or deliberate design; accepted/deferred real debt keeps overall yellow). COMPREHENSION AXIS (parallel, routing-gated behind Production-ready): `loom next --mode wiki` drains the code-primary wiki prose queue — author narrative prose citing source files (not intent UUIDs); `loom wiki --prose-check` certifies coverage + freshness + consistency gates green. It is a VECTOR, never a scalar: a graph can be Production-ready while Excellent is still ◐, so read every rung, then drive the focus.",
            "output_hygiene": {
...
mod tests {
    ...
    /// Indirect wiring (events/DI/registry/config-dispatch) is invisible to
    /// import analysis, so the protocol must teach the agent to hunt it and
    /// route a real coupling to a recorded edge + saga proof, never a mechanical
    /// completeness green. Pinned so the doctrine survives.
    #[test]
    fn indirect_wiring_is_taught_as_judgment_axis() {
        let rules = serde_json::to_string(&guide_json("brownfield")["deeper_rules"]).unwrap();
        assert!(
            rules.contains("INDIRECT WIRING") && rules.contains("manual") && rules.contains("saga"),
            "deeper rules must teach hunting indirect wiring + recording/proving it: {rules}"
        );
    }
    ...
    /// PM-BOUNDARY RATCHET: alignment must treat the graph as evidence, not as
    /// authority over an explicit product boundary call. This keeps cold LLMs
    /// from laundering a routed UI surface into "internal" just because nearby
    /// implementation code is backend/privacy machinery.
    #[test]
    fn seed_guide_teaches_pm_boundary_calls_override_graph_guesswork() {
        let blob = serde_json::to_string(&guide_json("seed")).unwrap();
        for phrase in [
            "decision aid, not the boss",
            "explicit PM boundary call",
            "privacy page",
            "inbound operator UI boundary",
            "routed Svelte surface",
            "--visibility user_visible --boundary inbound",
            "backend internals",
        ] {
            assert!(
                blob.contains(phrase),
                "seed/align guide must teach '{phrase}' for PM boundary calls"
            );
        }
    }
