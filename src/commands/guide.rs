...
fn brownfield() -> Vec<(&'static str, &'static str)> {
    vec![
        ...
        ("audit", "`loom smells` — derived suspicions the graph noticed for you: twin intents (split-brain), duplicated responsibility (tag collisions across unrelated code, with a weaker lexical fallback for under-tagged coded pairs), overlapping ownership, scatter, tangles, undeclared coupling, layering violations (imports pointing UP the declared `loom layer order` — a recorded relationship doesn't excuse direction; adjudicate a deliberate up-dependency with a decision note on the importing intent), vocab drift, rules never held against coded intents, happy-path-only feature groups (no sad/fallback behavior declared). OPEN findings GATE GREEN: once every queue is dry the compass routes phase=audit until `loom smells` returns zero. Refute or confirm each via its remedy; `independent`/a decision note is as valuable as a fix (scatter/tangle/happy-path adjudicate via `loom note add --intent|--file … --kind decision`; a later structural change re-opens the question). Per-file ownership questions: `loom codefile show <path>`. The report also DISCLOSES detector coverage (untagged coded intents mean duplicate detection is under-armed, not blind; layers in use with no declared order are a true layering blind spot) — a quiet report is only as good as its armed instruments."),
