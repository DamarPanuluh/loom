...
- **Autonomous adversarial review:** the ordinary Review lane combines low-confidence re-inspection with a fixed top-risk frontier of green Verdict revisions. `loom next` gives the LLM a hypothesis-first challenge contract; `loom challenge record` snapshots the exact revision, and credible counterexamples become Findings for independent Triage without silently rewriting the original Verdict.
...
- **Validation-specific proof strength:** the S3 call witness is earned by a validation's own evidence — an explicit `loom edge exercises <validation> <codefile> --locator <entry-symbol>` entry (the locator is required for S3; a bare file claim is diagnostic only), an entry point derived from its command (`cargo test --test …`, `cargo run --bin …`, scripts, binaries), or a compiler-owned Journey Exercises closure (public adapter plus optional operation `exercises` whose `observed_by` assertion passed on a local compiler-v6 `journey run`). Imported or deserialized run records cannot supply that assertion evidence. Compiler-v5 Journey proofs must be recompiled and rerun; schema v12 graphs do not need rebuilding. A sibling validation on the same intent never inherits another proof's reach, and the legacy intent-wide surface is recorded visibly as non-eligible fallback. `loom validation show` explains the grade with the exact source, file, entry symbol, reached symbol, and Journey operation provenance when present.
