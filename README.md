...
If you are an LLM with a short context window, start from `loom status` and follow its `one_turn` plan. It gives one lane, one role, the exact `loom guide --role …` command to adopt, and the exact `loom next --mode …` queue to pull. Alarms preempt that plan; otherwise ignore the other counters for the turn, finish exactly one recorded item, run `loom sync`/`loom export` when applicable, then ask `loom status` again.
...
3. Classify relationships as you inspect them. `RELATES_TO` items carry `kinds` and `classification_needed`; if the kind is still unknown, add a true judgment kind (`calls`, `inheritance`, `shares_state`, `doc_reference`, `manual`) while re-recording the verdict. Do **not** assert mechanical kinds (`imports`, `shares_file`, `shares_vocab`, `same_domain`) — sync/populate derives those — and never use `manual` just to silence churn. Unknown kinds stay conservative, so classifying honestly pays down future ceremony without hiding risk.
4. After any code change: `loom sync`. Change detection is **content-hash based** (checkout/rebase mtime churn never false-flags), and every invalidated edge gets a note naming the file that staled it. Sync narrows by symbol/import facts where it can; `loom impact` distinguishes actual already-synced edits from explicit hypothetical blast-radius previews, so drivers do not chase fake work.
5. `loom smells` surfaces what nobody asserted: twin intents, overlapping ownership, scattered responsibilities (clustered by directory), tangled files, undeclared coupling (imports the graph doesn't explain), layering violations (imports pointing UP the declared `loom layer order` over intent `--layer` labels — a recorded relationship doesn't excuse direction), recurrent trouble, happy-path-only features (no failure behavior declared), duplicated responsibility (tag collisions across unrelated code, with a weaker lexical fallback for under-tagged coded pairs), symbol-accountability gaps (public/risky symbols without precise ownership), vocab drift, unjourneyed surface (a user-visible behavior no consumer journey ever exercises end-to-end). Each finding carries its exact remedy command — and the redesign-shaped ones emit a `loom hypothesis add`, so a redesign gets *proven* before it becomes work. Open findings gate green: `phase=complete` requires zero, and adjudications (decision notes) re-open automatically when the structure changes under them.
6. Seed the quality packs `loom detect` recommends — `iso5055` (baseline, any code), `mobile`, `web-ui`, `service`, `data`, `concurrency`, `docker` — and `loom next --mode quality` serves every rule × coded-intent pair never measured. One `loom rule verdict` resolves each (it creates the edge; a verdict at component altitude covers descendants; `independent` = measured, doesn't apply).
7. Close out with `loom next --all`, prove intents with `loom validate`.
...
- `loom guide --mode seed` teaches the elicitation protocol: calibrate altitude to the user's fluency (a vague user gets *proposals to react to*, never "enumerate your features"), one question per inbox card and one landing (every crystallized answer is captured first, normalized, then becomes a `planned` intent, a vocab term, or a decision note), and **terminate on completeness, not exhaustion** — every question must close a gap the graph can enumerate; no open gap, no more questions. An empty graph's compass starts here (`phase=seed`).
...
- The graph is a **decision aid, not the boss**: graph/code evidence can challenge a claim, but an explicit PM boundary call is the product decision to record. If the user says a privacy page is an inbound operator UI boundary because it is a routed Svelte surface, align should encode that intent as user-visible inbound UI, not collapse it into backend internals.
...
- `loom door "<utterance>"` is the **entrance**: users don't speak in graph nouns, and their process isn't linear — a story, a complaint, a norm, a question, a decision, a risk, an interface gap, in any order. Door captures the raw utterance as an `InboxItem` first, then assembles routing context mechanically (what every plane already knows about the topic: intents by BM25, vocab/sagas/rules by token overlap, plus the compass pulse) and returns the **landing menu**. The LLM normalizes the card with `loom inbox normalize`, runs the proposed graph command, then marks the card routed/rejected/duplicate/deferred.
...
`loom saga add checkout.saga.yaml` declares the proof; `loom saga diagnose checkout-flow` runs the same journey without stamping graph verdicts and explains common failure roots (missing env, auth-like status failures, 404s, template/body mismatches, skipped dependent steps). If a step declares `auth.requires_scopes`, diagnose decodes the bearer JWT's `scope`/`scp`/`scopes` claims and names the exact missing scope. Once the failure is understood or the target is ready, `loom saga run checkout-flow` executes it and stamps the result into the graph with honest failure semantics: consecutive passing steps mark their `RELATES_TO` edge **passing with runtime evidence**; the boundary into a failing step goes **failing with the exact broken expectation** (`expected 200, got 502`) and lands in the fix queue; steps after the failure stay untouched — *never reached* is not *failing*. `loom sync` re-queues the saga whenever code behind a step intent changes, and the spec travels verbatim across a port — it speaks HTTP, not the implementation language.
...
Work items separate **discipline** from **queue**. A stale relationship in `loom next --mode fix` may be analyzer work, a review item may use analyzer or quality discipline, and a refactor advisory may use builder discipline — but the dispatch always tells the agent to keep pulling from the queue that served the item. That keeps weak-memory agents from adopting the right hat and then wandering into the wrong lane.
