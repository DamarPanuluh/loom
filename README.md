...
- `loom door "<utterance>"` is the **entrance**: users don't speak in graph nouns, and their process isn't linear — a story, a complaint, a norm, a question, in any order. Door captures the raw utterance as an `InboxItem` first, then assembles routing context mechanically (what every plane already knows about the topic: intents by BM25, vocab/sagas/rules by token overlap, plus the compass pulse) and returns the **landing menu**. The LLM normalizes the card with `loom inbox normalize`, runs the proposed graph command, then marks the card routed/rejected/duplicate/deferred.
